...
const CollapsibleItem = ({ title, children, isCheckAll, onCheckAllChange, isUnobtainable }) => {
...
const PlanList = ({ session }) => {
  ...
  const handleDownload = () => {
    const completedPlanIds = Object.keys(completedPlans).filter(id => completedPlans[id]);
        ...
        try {
          ...
          if (session) {
            ...
            for (let i = 0; i < uploadedPlanIds.length; i += batchSize) {
              const batch = uploadedPlanIds.slice(i, Math.min(i + batchSize, uploadedPlanIds.length));
  ...
  const { organizedPlans, totalObtainablePlans, totalUnobtainablePlans } = useMemo(() => {
    ...
    plans.forEach(plan => {
      const { item_type, item_name, section, name, location, plan_id, obtainable } = plan;
      ...
      organized[category][item_type][item_name][section].push({ name, location, planId: plan_id });
    });

    return { organizedPlans: organized, totalObtainablePlans: totalObtainable, totalUnobtainablePlans: totalUnobtainable };
  }, [plans]);
  ...
  const filteredPlans = useMemo(() => {
    ...
    ['obtainable', 'unobtainable'].forEach(category => {
      Object.entries(organizedPlans[category]).forEach(([itemType, itemNames]) => {
    ...
    <div className="plan-list">
      <div className={`content-wrapper ${isUploading ? 'blurred' : ''}`}>
        ...
        {['obtainable', 'unobtainable'].map(category => (
          <div key={category}>
            <h2 className={category === 'unobtainable' ? 'unobtainable-plans-title' : ''}>
              {category === 'obtainable' ? 'Obtainable' : 'Unobtainable'} Plans
            </h2>
            {Object.entries(filteredPlans[category]).map(([itemType, itemNames]) => (
              <CollapsibleItem 
                key={itemType} 
                title={itemType} 
                  ...
                  >
                    {Object.entries(sections).map(([section, items]) => (
                      <CollapsibleItem 
                        key={section} 
                        title={section}
                        isCheckAll={items.every(item => completedPlans[item.planId])}
                        onCheckAllChange={() => {
                          const allChecked = items.every(item => completedPlans[item.planId]);
                        ...
                        {items.map((item) => (
                          <div key={item.planId} className="plan-container-style">
                            <label>
                              ...
                              />
                              <span className={`plan-name-style ${category === 'unobtainable' ? 'unobtainable-plan' : ''}`}>
                                {item.name}
                              </span>
                            </label>
                            <div className="location-text-style">{item.location}</div>
                          </div>
                        ))}
                      </CollapsibleItem>
                    ))}
